...
$(document).ready(function() {
    ...
    $(".comm_write_btn_wrap .btn_white").on("click", function() {
	    ...
	    var startdate = $("input[name='startdate']").val(); // 시작 날짜 입력값 가져옴
	    var enddate = $("input[name='enddate']").val(); // 종료 날짜 입력값 가져옴
	    var content = $("#summernote").summernote("code"); // 내용 입력값 가져옴
	    var photo =  $(".thumb img").attr("src"); // 사진 첨부 여부 확인
        
	    if (title.trim() !== "" || startdate.trim() !== "" || enddate.trim() !== "" || content.trim() !== "" || photo.trim() !== "") {
...
$(document).ready(function() {
	...
	function imageUploader(file, el) {
		...
		$.ajax({                                                              
            ...
            <div class="main_content">
                ...
                <div class="write_box">
                    <form role="form" action="${ctx}/event/write" method="post" enctype="multipart/form-data">
                        <div class="write_tit">
 ...
 <script>
 $(document).ready(function() {
	// 시작 날짜(input 태그)가 변경될 때마다 종료 날짜의 최솟값을 설정합니다.
    $("input[name='startdate']").on("change", function() {
        // 시작 날짜의 값을 가져옵니다.
        var startDate = new Date($(this).val());
        
        // 종료 날짜(input 태그)의 최솟값을 설정합니다.
        $("input[name='enddate']").attr("min", formatDate(startDate));
        
        // 종료 날짜의 값을 확인하여 최솟값보다 작은 경우 값을 초기화합니다.
    ...
    // 종료 날짜(input 태그)가 변경될 때마다 시작 날짜의 최댓값을 설정합니다.
    $("input[name='enddate']").on("change", function() {
        // 종료 날짜의 값을 가져옵니다.
        var endDate = new Date($(this).val());
        
        // 시작 날짜(input 태그)의 최댓값을 설정합니다.
        $("input[name='startdate']").attr("max", formatDate(endDate));
        
        // 시작 날짜의 값을 확인하여 최댓값보다 큰 경우 값을 초기화합니다.
